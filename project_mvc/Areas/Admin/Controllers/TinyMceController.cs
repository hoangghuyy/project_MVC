    ...
    public class TinyMceController : Controller
    {
        ...
        private string MakeUniqueFilename(string dir, string filename)
        {
            ...
            if (System.IO.File.Exists(Path.Combine(dir, ret)))
            {
                ...
                ret = ConvertAscii(name) + "-copy" + i.ToString() + Path.GetExtension(filename);
            }
            else
            {
                var name = Path.GetFileNameWithoutExtension(filename);
                ret = ConvertAscii(name) + Path.GetExtension(filename);
            }
            return ret;
        }
        ...
        public static string ConvertAscii(string unicode)
        {
            try
            {
        ...
        public static string NewUnicodeToAscii(string unicode)
        {
            try
            {
                ...
                unicode = Regex.Replace(unicode, @"[đ]","d", RegexOptions.IgnoreCase);
                unicode = Regex.Replace(unicode, @"[ýỳỷỹỵ]","y", RegexOptions.IgnoreCase);
                unicode = Regex.Replace(unicode, @"[^a-z0-9 \-]", "", RegexOptions.IgnoreCase);
                return unicode;
            }
            catch
            {
